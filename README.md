...
原生混淆中，Android 四大组件(Activity,Service等)、自定义View等只要在xml文件中用到的类，都是不会被混淆的，这样就会有以下2个缺点

- 通过反编译，可以轻松看到真实的类名及包名

- 有过上架`Google Play`
  商店的同学应该知道，如果之前的包被下架或封号，想要同套代码再次上架，那几乎是不可能的，99%都是再次封号，很大一部分原因就是以上说到的类不会被混淆，
  Google只要通过技术手段对比下，基本结果就是包重复，从而导致再次封号
  
针对第二点，如果我们想要再次上架，就必须要更改四大组件、自定义View等的`包名+类名`，然而，如果手动去完成这项任务，估计会累死一个程序员，于是乎，`XmlClassGuard`
就来了，通过插件去完成手工的活，一个任务便可搞定
